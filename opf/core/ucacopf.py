...
from .acopf_exp import (
    cnst_slack_va_exp,
    cnst_thermal_branch_from_exp,
    cnst_thermal_branch_to_exp,
    cnst_ohm_pf_from_exp,
    cnst_ohm_pf_to_exp,
    cnst_ohm_qf_from_exp,
    cnst_ohm_qf_to_exp,
    cnst_p_balance_exp,
    cnst_q_balance_exp,
    cnst_dva_exp,
    define_sets_balance_exp,
    obj_cost_exp,
    bound_qg_exp,
    bound_vm_exp,
)
from .ucacopf_exp import (
    bound_pg_exp,
    cnst_gav_exp,
    const_pg_max_uc_exp,
    const_pg_min_uc_exp,
...
class UCACOPFModel(NormalOPFModel):
    """ UC-AC-OPF optimization model class.  
    """
    def __init__(self, model_type):
        super().__init__(model_type)

    def _build_model(self) -> None:
        """ Define the (abstract) UC-AC-OPF optimization model. 
        ...
        # I.    Parameters
        # ====================
        self.model.uc_init = pyo.Param(self.model.G, within=pyo.Binary, mutable=True)
        ...
        self.model.gav = pyo.Param(self.model.G, within=pyo.Binary, mutable=True)
        ...
        # # II.    Variables
        # # ====================
        self.model.uc = pyo.Var(self.model.G, initialize=self.model.uc_init, bounds=(0, 1), within=pyo.Binary) # generator unit commitment, binary
        ...
        # ====================
        # III.f Generator Availability
        # ====================
        self.model.cnst_gav = pyo.Constraint(self.model.G, rule=cnst_gav_exp)

        # ====================
        # III.g Generator Capability
        # ====================
        self.model.const_pg_max_uc = pyo.Constraint(self.model.G, rule=const_pg_max_uc_exp)
        self.model.const_pg_min_uc = pyo.Constraint(self.model.G, rule=const_pg_min_uc_exp)
        self.model.const_qg_max_uc = pyo.Constraint(self.model.G, rule=const_qg_max_uc_exp)
        self.model.const_qg_min_uc = pyo.Constraint(self.model.G, rule=const_qg_min_uc_exp)
